...
public class MyMediaController extends FrameLayout {
    ...
    public MyMediaController(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.MyMediaController);
        mScalable = a.getBoolean(R.styleable.MyMediaController_mvv_scalable, false);
        a.recycle();
        init(context);
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SHOW_ERROR: //5
                    ...
                    break;
                case HIDE_LOADING: //
                    //第一次缓存完毕后，显示中央播放按钮，但后来缓存完毕时不再显示，因为后来是自动播放的
                    if (firstPlay) {
                        showCenterView(R.id.center_play_btn);
                        firstPlay = false;
                    } else {
    ...
    private OnClickListener mPauseListener = new OnClickListener() {
        public void onClick(View v) {
            if (mCenterPlayButton.getVisibility() == View.VISIBLE) {
